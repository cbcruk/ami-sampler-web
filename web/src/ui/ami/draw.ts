...
export function bevel(ctx: CanvasRenderingContext2D, r: Rect, light: string, dark: string, w = 2): void {
  ctx.fillStyle = light;
...
export function button(
  ...
  text(ctx, label, r.x + r.w / 2, r.y + r.h / 2 - r.h * 0.32, Math.round(r.h * 0.55), txtColor, "center");
}
...
export function hSlider(ctx: CanvasRenderingContext2D, r: Rect, t: number, dragging: boolean, disabled = false): void {
  checkerTrack(ctx, r);
  if (disabled) {
...
export function checkbox(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, checked: boolean): void {
