...
import { type Rect, inRect, hSlider, button as drawButton, checkbox as drawCheckbox, text, bevel } from "./draw";
...
export class Checkbox implements Widget {
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    text(ctx, this.o.label, r.x + s + 6, r.y + r.h / 2 - 7, 14, this.o.disabled ? "#7a7a90" : AMI_WHT, "left");
  }
  hit(x: number, y: number): boolean {
    return !this.o.disabled && inRect(this.rect, x, y);
  }
