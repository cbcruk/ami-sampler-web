...
export function parseBrr(bytes: ArrayBuffer): SampleData {
  ...
  const coeff = (): { a: number; b: number } => {
    switch (filterFlag) {
      case 1: return { a: fixedMath(tmp[0], 15, 16), b: 0 };
      case 2: return { a: fixedMath(tmp[0], 61, 32), b: fixedMath(tmp[1], 15, 16) };
      case 3: return { a: fixedMath(tmp[0], 115, 64), b: fixedMath(tmp[1], 13, 16) };
